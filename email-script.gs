 ...
 *   - 09/25/19 (zespirit): Added more documentation and "Control Spreadsheet"
 ...
 * ---------------------------------
 *
 * The anonymous ID mappings spreadsheet contains two columns:
 *
 *   1. Column A is the emails for each student.
 *   2. Column B is the associated anonymous ID for the student on the same row.
 *      Anonymous IDs do not need to be numerical, though the `anonymization-
 *      script.gs` provided with GrblGrader generates numerical anonymous IDs.
 *
 *
 * Grading Spreadsheet
 * -------------------
 *
 * The grading spreadsheet may be formatted in many ways, with the following
 * restrictions:
 *
 *   1. Column A contains anonymous IDs. (These IDs should be associated with a
 *      student identifier via the Anonymous ID Mappings Spreadsheet.)
 *   2. Row #4 contains "column tags" for each column. These tags associate
 *      values in a given column with a substitution location on the GRBL
 *      template. All column tags should be unique. (See the "Report Templates"
 *      section for more information on how to use column tags in GRBL
 *      templates.)
 *
 *
 * ================
 ...
 * tag", which are set on the 4th row on the grading spreadsheet. A value
 * is mapped from a column tag using GRBL's "replacement operator":
 *
 *   ```
 *   {{column-tag-goes-here}}
 *   ```
 *
 * For example, here's a really simple "report template" that maps values from
 * a single column with the tag `final-score`:
 *
 *   ```
 *   Your grade is:
 *
